import logo_maclienson from './logo_maclienson.png';
// import crosin from './crosin.png';
// import ecapsule from './ecapsule.png';
// import paracetamol from './paracetamol.png';
 import vision from './vision.png';
 import vision_1 from './vision_1.png';
 import mission_1 from './mission_1.png';
 import mission_2 from './mission_2.png';
 import values_1 from './values_1.png';
 import values_2 from './values_2.png';
 import login_2 from './login_2.png';
 import rh_a1 from './rh_a1.jpg';
 import rh_a2 from './rh_a2.jpg';
 import rh_a3 from './rh_a3.png';
 import rh_a4 from './rh_a4.jpg';
import digdis_1 from './digdis_1.jpg';
import digdis_2 from './digdis_2.jpg';
import digdis_3 from './digdis_3.png';
import rh_myo from './rh_myo.png';
import digdis_w from './digdis_w.png';
import main_c5 from './main_c5.jpg';
import main_c6 from './main_c6.png';
 import main_p from './main_p.png';
 import main_c2 from './main_c2.png';
 import main_c3 from './main_c3.png';
 import main_c4 from './main_c4.png';
 import main_c7 from './main_c7.jpeg';
 import main_pw from './main_pw.webp';
// import syrup_m from './syrup_m.png';
// import syrup_m_2 from './syrup_m_2.png';
 import main_cp from './main_cp.png';
// import protien_c from './protien_c.png';
// import tablet_c from './tablet_c.png';
// import capsule_c from './capsule_c.png';
// import syrup_c from './syrup_c.png';
 
// import paraffine_500 from './paraffine_500.png';
// import B12_neuro_cat from './B12_neuro_cat.png';
// import pan_d_cat from './pan_d_cat.png';
// import beneprotein_bs from './beneprotein_bs.png';
// import cetzine_bs from './cetzine_bs.png';
// import revital_h_bs from './revital_h_bs.png';
// import sterellium_med_bs from './sterellium_med_bs.png';
// import beneprotein_back_bs from './beneprotein_back_bs.png';
 import featured from './featured.png';
 import login_img from './login_img.png';
 import register_img from './register_img.png';
// import azithral_500 from './azithral_500.png';
// import naproxen_c from './naproxen_c.png';
// import nexito_10_c from './nexito_10_c.png';
// import alprazolam_c from './alprazolam_c.png';
// import taxim_o_c from './taxim_o_c.png';
import sanitizer_c from './sanitizer_c.png';
 import medicine_cat from './medicine_cat.png';
 import healthcare_cat from './healthcare_cat.png';
import logo_maclienson_br from './logo_maclienson_br.png';
import tabletsrx from './tabletsrx.png';
import macnurish_p from './macnurish_p.png';
import syrup from './syrup.png';
import nurturmac from './nurturmac.png';
import syndrova_met_sr_t from './syndrova_met_sr_t.png';
import flavona_forte_sy from './flavona_forte_sy.png';
import flavona_forte_sy_b from './flavona_forte_sy_b.png';

       

    




export const assets = {
  logo_maclienson,
  logo_maclienson_br,
  // crosin,
  // ecapsule,
  // paracetamol,
  syrup,
  // syrup_m,
  main_p,
  main_c2,
  main_c3,
  main_c4,
  main_pw,
  main_c5,
  main_c6,
  main_c7,
  // syrup_m_2,
  main_cp,
  // protien_c,
  // tablet_c,
  // capsule_c,
  // syrup_c,
   sanitizer_c,
  medicine_cat,
   healthcare_cat,
  // paraffine_500,
  // B12_neuro_cat,
  // pan_d_cat,
  // beneprotein_bs,
  // cetzine_bs,
  // revital_h_bs,
  // sterellium_med_bs,
  // beneprotein_back_bs,
  featured,
  login_img ,
  register_img,
  vision,
  vision_1,
  mission_1,
  mission_2,
  values_1,
  values_2,
  login_2,
  rh_a1,
  rh_a2,
  rh_a3,
  rh_a4,
  rh_myo,
  digdis_1,
  digdis_2,
  digdis_3,
  digdis_w,
  // azithral_500,
  // alprazolam_c,
  // taxim_o_c,
  tabletsrx,
  macnurish_p,
  nurturmac,
  syndrova_met_sr_t,
  flavona_forte_sy,
  flavona_forte_sy_b
  
};

export const products = [
  {
    id: "p27",
    name: " Macnurish",
    price: 0,
    image:  macnurish_p,
    category: "Women",
    subcategory: "Nutritional Supplement",
    date: "2025-04-10",
    bestseller: true,
    description: "Macnurish Sachet is a sugar-free prenatal supplement that supports fetal development and maternal health with key nutrients like L-Arginine, Lycopene, L-Methyl Folate, and Zinc.",
    
  },
  {
    id: "p28",
    name: "Nurturmac",
    price: 0,
    image: nurturmac,
    category: "Women",
    subcategory: "Nutritional Supplement",
    date: "2025-04-10",
    bestseller: true,
    description:" Nurturmac is a prenatal softgel capsule enriched with L-Methylfolate, Vitamin B6, B12, Biotin, and DHA to support maternal health and fetal development."
  },
  {
    id: "p29",
    name: "SYNDROVA-MET SR Tablets",
    price: 0,
    image: syndrova_met_sr_t,
    category: "Women",
    subcategory: "Nutritional Supplement",
    date: "2025-04-10",
    bestseller: true,
    description: "Manage your health with Syndrova-Met SR, a sustained-release tablet containing Myo Inositol and Metformin Hydrochloride."
  },
  {
    id: "p30",
    name: "Flavona forte Syrup",
    price: 0,
    image: flavona_forte_sy,
    category: "Women",
    subcategory: "Herbal Supplement",
    date: "2025-04-10",
    bestseller: true,
    description: "Flavona Forte Syrup is an advanced Ayurvedic formula crafted for women's wellness, helping to correct hormonal imbalance, boost vitality, and enhance overall well-being."
  },
  {
    id: "p31",
    name: "tabletsrx",
    price: 0,
    image: tabletsrx,
    category: "Women",
    subcategory: "Analgesic",
    date: "2025-04-10",
    bestseller: true,
    description: "tabletsrx  effective for treating fever and mild to moderate pain."
  },
  {
    id: "p32",
    name: "tabletsrx",
    price: 0,
    image: tabletsrx,
    category: "Women",
    subcategory: "Analgesic",
    date: "2025-04-10",
    bestseller: true,
    description: "tabletsrx  effective for treating fever and mild to moderate pain."
  },
  {
    id: "p33",
    name: "tabletsrx",
    price: 0,
    image: tabletsrx,
    category: "Women",
    subcategory: "Analgesic",
    date: "2025-04-10",
    bestseller: true,
    description: "tabletsrx  effective for treating fever and mild to moderate pain."
  },
  {
    id: "p34",
    name: "tabletsrx",
    price: 0,
    image: tabletsrx,
    category: "Women",
    subcategory: "Analgesic",
    date: "2025-04-10",
    bestseller: true,
    description: "tabletsrx  effective for treating fever and mild to moderate pain."
  },
  {
    id: "p35",
    name: "tabletsrx",
    price: 0,
    image: tabletsrx,
    category: "Women",
    subcategory: "Analgesic",
    date: "2025-04-10",
    bestseller: true,
    description: "tabletsrx  effective for treating fever and mild to moderate pain."
  },
  {
    id: "p36",
    name: "tabletsrx",
    price: 0,
    image: tabletsrx,
    category: "Women",
    subcategory: "Analgesic",
    date: "2025-04-10",
    bestseller: true,
    description: "tabletsrx  effective for treating fever and mild to moderate pain."
  },
  {
    id: "p37",
    name: "tabletsrx",
    price: 0,
    image: tabletsrx,
    category: "Women",
    subcategory: "Analgesic",
    date: "2025-04-10",
    bestseller: true,
    description: "tabletsrx  effective for treating fever and mild to moderate pain."
  },
  {
    id: "p38",
    name: "tabletsrx",
    price: 0,
    image: tabletsrx,
    category: "Women",
    subcategory: "Analgesic",
    date: "2025-04-10",
    bestseller: true,
    description: "tabletsrx  effective for treating fever and mild to moderate pain."
  },
  {
    id: "p39",
    name: "tabletsrx",
    price: 0,
    image: tabletsrx,
    category: "Women",
    subcategory: "Analgesic",
    date: "2025-04-10",
    bestseller: true,
    description: "tabletsrx  effective for treating fever and mild to moderate pain."
  },
  {
    id: "p40",
    name: "tabletsrx",
    price: 0,
    image: tabletsrx,
    category: "Women",
    subcategory: "Analgesic",
    date: "2025-04-10",
    bestseller: true,
    description: "tabletsrx  effective for treating fever and mild to moderate pain."
  },
  {
    id: "p41",
    name: "tabletsrx",
    price: 0,
    image: tabletsrx,
    category: "Women",
    subcategory: "Analgesic",
    date: "2025-04-10",
    bestseller: true,
    description: "tabletsrx  effective for treating fever and mild to moderate pain."
  },
  {
    id: "p42",
    name: "tabletsrx",
    price: 0,
    image: tabletsrx,
    category: "Women",
    subcategory: "Analgesic",
    date: "2025-04-10",
    bestseller: true,
    description: "tabletsrx  effective for treating fever and mild to moderate pain."
  },
  {
    id: "p43",
    name: "tabletsrx",
    price: 0,
    image: tabletsrx,
    category: "Women",
    subcategory: "Analgesic",
    date: "2025-04-10",
    bestseller: true,
    description: "tabletsrx  effective for treating fever and mild to moderate pain."
  },
  {
    id: "p44",
    name: "tabletsrx",
    price: 0,
    image: tabletsrx,
    category: "Women",
    subcategory: "Analgesic",
    date: "2025-04-10",
    bestseller: true,
    description: "tabletsrx  effective for treating fever and mild to moderate pain."
  },
  {
    id: "p45",
    name: "tabletsrx",
    price: 0,
    image: tabletsrx,
    category: "Women",
    subcategory: "Analgesic",
    date: "2025-04-10",
    bestseller: true,
    description: "tabletsrx  effective for treating fever and mild to moderate pain."
  },
  {
    id: "p46",
    name: "tabletsrx",
    price: 0,
    image: tabletsrx,
    category: "Women",
    subcategory: "Analgesic",
    date: "2025-04-10",
    bestseller: true,
    description: "tabletsrx  effective for treating fever and mild to moderate pain."
  },
  
  // {
  //   id: "p1",
  //   name: "Crosin",
  //   price: 199,
  //   image: crosin,
  //   category: "Unisex",
  //   subcategory: "Analgesic",
  //   date: "2025-04-10",
  //   bestseller: true,
  //   description: "Crosin  commonly used for pain relief and fever reduction."
  // },
  // {
  //   id: "p2",
  //   name: "Vitamin E Capsule",
  //   price: 500,
  //   image: ecapsule,
  //   category: "Unisex",
  //   subcategory: "Supplement",
  //   date: "2025-04-10",
  //   bestseller: true,
  //   description: "Vitamin E Capsules  support skin health and act as antioxidants."
  // },
  // {
  //   id: "p3",
  //   name: "Paracetamol",
  //   price: 225,
  //   image: paracetamol,
  //   category: "Women",
  //   subcategory: "Analgesic",
  //   date: "2025-04-10",
  //   bestseller: true,
  //   description: "Paracetamol  effective for treating fever and mild to moderate pain."
  // },
  // {
  //   id: "p4",
  //   name: "Beneprotein Powder",
  //   price: 399,
  //   image: beneprotein_bs,
  //   category: "Unisex",
  //   subcategory: "Protein Supplement",
  //   date: "2025-04-10",
  //   bestseller: false,
  //   description: "Beneprotein  high-quality whey protein for nutritional support."
  // },
  // {
  //   id: "p5",
  //   name: "Pan_d",
  //   price: 185,
  //   image: pan_d_cat,
  //   category: "Unisex",
  //   subcategory: "Gastrointestinal",
  //   date: "2025-04-10",
  //   bestseller: true,
  //   description: "Pan-D  used for acidity, gastric reflux, and peptic ulcers."
  // },
  // {
  //   id: "p6",
  //   name: "Paraffine_500",
  //   price: 250,
  //   image: paraffine_500,
  //   category: "Women",
  //   subcategory: "Laxative",
  //   date: "2025-04-10",
  //   bestseller: false,
  //   description: "Paraffin 500  a mild laxative for relieving constipation."
  // },
  // {
  //   id: "p7",
  //   name: "B12 neuro",
  //   price: 500,
  //   image: B12_neuro_cat,
  //   category: "Unisex",
  //   subcategory: "Vitamin Supplement",
  //   date: "2025-04-10",
  //   bestseller: true,
  //   description: "B12 Neuro  vitamin B12 supplement to support nerve health."
  // },
  // {
  //   id: "p8",
  //   name: "Revital_h",
  //   price: 500,
  //   image: revital_h_bs,
  //   category: "Unisex",
  //   subcategory: "Multivitamin",
  //   date: "2025-04-10",
  //   bestseller: true,
  //   description: "Revital H  daily multivitamin and mineral supplement to boost energy."
  // },
  // {
  //   id: "p9",
  //   name: "Cetzine",
  //   price: 120,
  //   image: cetzine_bs,
  //   category: "Unisex",
  //   subcategory: "Antihistamine",
  //   date: "2025-04-10",
  //   bestseller: false,
  //   description: "Cetzine  antihistamine used to treat allergies, hay fever, and hives."
  // },
  // {
  //   id: "p10",
  //   name: "Sanitizer",
  //   price: 130,
  //   image: sanitizer_c,
  //   category: "Unisex",
  //   subcategory: "Disinfectant",
  //   date: "2025-04-10",
  //   bestseller: true,
  //   description: "Hand Sanitizer  alcohol-based disinfectant for hygiene and protection."
  // },
  // {
  //   id: "p11",
  //   name: "Sterellium_med",
  //   price: 135,
  //   image: sterellium_med_bs,
  //   category: "Unisex",
  //   subcategory: "Disinfectant",
  //   date: "2025-04-10",
  //   bestseller: false,
  //   description: "Sterillium Med  medical-grade hand disinfectant with rapid action."
  // },
  // {
  //   id: "p12",
  //   name: "Taxim-O",
  //   price: 70,
  //   image: taxim_o_c,
  //   category: "Unisex",
  //   subcategory: "Antibiotic",
  //   date: "2025-04-10",
  //   bestseller: true,
  //   description: "Taxim-O  antibiotic used to treat bacterial infections."
  // },
  // {
  //   id: "p13",
  //   name: "Alprazolam",
  //   price: 90,
  //   image: alprazolam_c,
  //   category: "Unisex",
  //   subcategory: "Anxiolytic",
  //   date: "2025-04-10",
  //   bestseller: false,
  //   description: "Alprazolam  prescribed for anxiety disorders and panic attacks."
  // },
  // {
  //   id: "p14",
  //   name: "Azithral_500",
  //   price: 50,
  //   image: azithral_500,
  //   category: "Unisex",
  //   subcategory: "Antibiotic",
  //   date: "2025-04-10",
  //   bestseller: true,
  //   description: "Azithral 500  broad-spectrum antibiotic used to treat infections."
  // },
  // {
  //   id: "p15",
  //   name: "Nexito_10_c",
  //   price: 60,
  //   image: nexito_10_c,
  //   category: "Unisex",
  //   subcategory: "Antidepressant",
  //   date: "2025-04-10",
  //   bestseller: true,
  //   description: "Nexito 10  antidepressant prescribed for depression and anxiety."
  // },
  // {
  //   id: "p16",
  //   name: "Naproxen_c",
  //   price: 140,
  //   image: naproxen_c,
  //   category: "Unisex",
  //   subcategory: "NSAID",
  //   date: "2025-04-10",
  //   bestseller: false,
  //   description: "Naproxen  nonsteroidal anti-inflammatory drug used for pain relief."
  // },
];
