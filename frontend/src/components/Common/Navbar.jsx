  ...
  return (
    <>
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ease-in-out ${
        ...
        <nav className="container mx-auto px-4 lg:px-12 flex items-center justify-between text-primary-950">
          
          {/* Left - Logo & Legal Info */}
          <div className="flex flex-col items-center transition-all duration-500 flex-shrink-0">
            <Link to="/" className="flex-shrink-0 mb-1">
              <img src={assets.logo_maclienson_br} alt="Maclienson Logo" className={`w-auto object-contain transition-all duration-500 ${isScrolled ? 'h-8 lg:h-10' : 'h-10 md:h-14 lg:h-[4.5rem]'}`} />
            </Link>
            
            <div className={`hidden sm:flex flex-col items-center text-center leading-[1.3] transition-all duration-500 overflow-hidden ${isScrolled ? 'h-0 opacity-0 mt-0' : 'h-auto opacity-100 mt-1'}`}>
              <div className="text-[6.5px] md:text-[7.5px] uppercase tracking-[0.25em] font-bold text-primary-900 flex flex-col mb-1">
          ...
          {/* Right - Icons */}
          <div className="flex items-center space-x-3 sm:space-x-5 lg:space-x-6 flex-shrink-0">
            {user?.role === 'admin' && (
